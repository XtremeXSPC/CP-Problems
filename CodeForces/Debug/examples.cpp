...
//===----------------------------------------------------------------------===//
/* Advanced Macro System with C++23 Features */
...
// Advanced FOR loop system with C++20 ranges:
...
template <typename F>
I64 binary_search(F&& predicate, I64 ok, I64 ng) {
  while (std::abs(ok - ng) > 1) {
    I64 mid = ok + (ng - ok) / 2;  // Avoid overflow
    (predicate(mid) ? ok : ng) = mid;
  }
  return ok;
}

template <typename F>
F64 binary_search_real(F&& predicate, F64 ok, F64 ng, I32 iterations = 100) {
  FOR(iterations) {
    F64 mid = ok + (ng - ok) / 2;
    (predicate(mid) ? ok : ng) = mid;
  }
...
//===----------------------------------------------------------------------===//
/* Namespace Usage and Final Setup */
