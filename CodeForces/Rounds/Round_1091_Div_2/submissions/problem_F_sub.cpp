 ...
 * @generated: 2026-04-07 18:08:13
 * @source: problem_F.cpp
 * @author: Costantino Lombardi
 *
 * @brief: Codeforces Round 1091 (Div. 2) - Problem F
 */
//===----------------------------------------------------------------------===//
...
auto freq(I32 v, I32 x) -> I64 {
  const I32 m = (x - 1 - v) / 2;
  if (x - 1 < v) return 0;
  return calc(v, m) << std::popcount(as<U32>(v));
}

void solve() {
  INT(x1, x2);

  I64 mn = Limits<I64>::max();
  I32 v  = 0;

  FOR(i, x1) {
    const I64 cur = freq(i, x2);
    if (cur < mn) {
      mn = cur;
      v  = i;
      if (mn == 0) break;
