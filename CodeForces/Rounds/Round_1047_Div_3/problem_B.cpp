...
/**
 * @file: problem_B.cpp
 * @brief Codeforces Round 1047 Div. 3 - Problem B
...
void solve() {
  ...
  V_i originalPerm(sequenceLength);
  V_i complementPerm(sequenceLength);
  
  // Read the input permutation.
  for (auto idx = 0; idx < sequenceLength; ++idx) {
    cin >> originalPerm[idx];
  }
  
  // Compute the complement permutation.
  const int totalSum = sequenceLength + 1;
  for (auto idx = 0; idx < sequenceLength; ++idx) {
    complementPerm[idx] = totalSum - originalPerm[idx];
  }
  
  // Output result with proper formatting.
  for (auto idx = 0; idx < sequenceLength; ++idx) {
    cout << complementPerm[idx];
    cout << (idx == sequenceLength - 1 ? '\n' : ' ');
  }
  
  debug(originalPerm, complementPerm);
}
