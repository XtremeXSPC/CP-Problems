...
/**
 * @file: problem_A.cpp
 * @brief Codeforces Round 1047 Div. 3 - Problem A
...
void solve() {
  ll num_iterations, final_num;
  cin >> num_iterations >> final_num;

  ll intermediate_value = final_num * (1LL << (num_iterations - 1));

  if (intermediate_value % 6 == 4) {
    // If the condition is met, this path is valid and matches the sample cases.
    cout << (intermediate_value - 1) / 3 << '\n';
  } else {
    // Otherwise, fall back to the always-valid path of k 'even' reversals.
    cout << (final_num * (1LL << num_iterations)) << '\n';
  }
}
...
auto main() -> int {
  ...
  int T = 1;
  cin >> T;
  // Using a C++20 range-based for loop for modern iteration
