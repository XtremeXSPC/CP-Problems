...
/**
 * @file: problem_B.cpp
 * @brief Codeforces Round 1048 (Div. 2) - Problem B
...
void solve() {
  ll oven_count, total_seconds;
  cin >> oven_count >> total_seconds;

  V_ll production_rates(oven_count);
  for (auto& rate : production_rates) {
    cin >> rate;
  }

  // Sort production rates in descending order.
  ranges::sort(production_rates, ranges::greater());

  // Determine number of useful collections (most productive ones).
