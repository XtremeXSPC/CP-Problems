 ...
 * @details: Optimized and refactored version using modern C++ idioms
 *           and patterns. Achieves better performance and readability.
 *
 * @status: PASSED
 */
//===----------------------------------------------------------------------===//
...
private:
  ...
  auto execute_graph_search() -> MaybeSolution {
    // First handle leaf vertices (degree 1).
    for (size_t value_id = 0; value_id < compressor.unique_count(); ++value_id) {
      if (cached_coprime_degrees[value_id] == 1) {
        // This value has exactly one coprime
        int leaf_index = cached_occurrences[value_id][0]; // Use first occurrence.

        for (int partner = 1; partner <= spec.element_count; ++partner) {
          if (partner != leaf_index && gcd(cached_original_values[value_id], spec.sequence[partner]) == 1) {
    ...
    for (int primary_index : sorted_indices) {
      ...
      for (int partner = 1; partner <= spec.element_count && static_cast<int>(candidates.size()) < SEARCH_WIDTH; ++partner) {
      ...
      for (int secondary_index : candidates) {
        int primary_id   = sequence_to_compressed_id[primary_index];
        int secondary_id = sequence_to_compressed_id[secondary_index];

        if (primary_id == -1 || secondary_id == -1)
          continue;

        analyzer->modify_element_presence(cached_squarefree_factors[primary_id], -1);
  ...
  auto process_leaf_vertex(int leaf_index) -> MaybeSolution {
    ...
    int partner_id = sequence_to_compressed_id[partner_found];
    if (partner_id == -1)
      return nullopt;

    // Temporarily remove both from the graph.
    analyzer->modify_element_presence(cached_squarefree_factors[leaf_id], -1);
    analyzer->modify_element_presence(cached_squarefree_factors[partner_id], -1);

    // Find second coprime pair.
    auto [third, fourth] = filtered_coprime_search(leaf_index, partner_found);

    if (third != -1) {
      // Solution found.
      return Quadruple{leaf_index, partner_found, third, fourth};
    }
    ...
    // CRITICAL: A leaf that can't form a solution blocks everything.
    // We should signal this as a definitive no-solution case.
    return nullopt;
  }
