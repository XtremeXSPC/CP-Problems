...
int main() {
  ...
  TreeMonoid<TreeCore<I64>, AddMonoid<I64>> sum(tree, val);
  assert(sum.path_prod(2, 4) == 140);
  assert(sum.subtree_prod(3) == 90);
  sum.set(3, 7);
  assert(sum.path_prod(2, 4) == 107);
  sum.multiply(4, 1);
  assert(sum.subtree_prod(3) == 58);

  using F = Pair<I64, I64>;
  Vec<F> affine{{2, 1}, {3, 4}, {5, 6}, {7, 8}, {11, 12}};
  TreeMonoid<TreeCore<I64>, AffineMonoid<I64>> comp(tree, affine);
  ...
  TreeMonoid<TreeCore<I64>, AddMonoid<I64>, true> edge_sum(tree, [&](I32 id) {
    return I64(id + 1);
  });
  assert(edge_sum.path_prod(2, 4) == 2 + 3 + 4);
  edge_sum.set(3, 100);
  assert(edge_sum.path_prod(2, 4) == 2 + 3 + 100);
  assert(edge_sum.subtree_prod(1) == 2 + 3 + 100);
  return 0;
}
