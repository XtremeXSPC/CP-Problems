...
const char* latestFeatures[] = {
    ...
    "Added opt defaults via opt<T>(key/index, default_val); check unused opts when using has_opt or default opt (turn off this check with "
    "suppressEnsureNoUnusedOpt()).",
    "For checker added --group and --testset command line params (like for validator), use checker.group() or checker.testset() to get "
    "values",
    "Added quitpi(points_info, message) function to return with _points exit code 7 and given points_info",
    "rnd.partition(size, sum[, min_part=1]) returns random (unsorted) partition which is a representation of the given `sum` as a sum of "
    "`size` positive integers (or >=min_part if specified)",
    ...
    "Reformatted",
    "Use setTestCase(i) or unsetTestCase() to support test cases (you can use it in any type of program: generator, interactor, validator "
    "or checker)",
    ...
    "Use registerGen(argc, argv, 1) to develop new generator, use registerGen(argc, argv, 0) to compile old generators (originally created "
    ...
    "Use -DENABLE_UNEXPECTED_EOF to enable special exit code (by default, 8) in case of unexpected eof. It is good idea to use it in "
    "interactors",
...
inline bool doubleCompare(double expected, double result, double MAX_DOUBLE_ERROR) {
  ...
  if (__testlib_isNaN(expected)) {
    ...
    double minv = __testlib_min(expected * (1.0 - MAX_DOUBLE_ERROR), expected * (1.0 + MAX_DOUBLE_ERROR));
    double maxv = __testlib_max(expected * (1.0 - MAX_DOUBLE_ERROR), expected * (1.0 + MAX_DOUBLE_ERROR));
...
public:
  ...
  int next(int from, int to) { return int(next((long long)to - from + 1) + from); }

  /* Returns random value in range [from,to]. */
  unsigned int next(unsigned int from, unsigned int to) { return (unsigned int)(next((long long)to - from + 1) + from); }

  /* Returns random value in range [from,to]. */
  long long next(long long from, long long to) { return next(to - from + 1) + from; }
...
pattern::pattern(std::string s) : s(s), from(0), to(0) {
  ...
  if (seps.size() == 0 && firstClose + 1 == (int)s.length() && __pattern_isCommandChar(s, 0, '(')
...
enum TResult { _ok = 0, _wa = 1, _pe = 2, _fail = 3, _dirt = 4, _points = 5, _unexpected_eof = 8, _partially = 16 };

enum TTestlibMode { _unknown, _checker, _validator, _generator, _interactor, _scorer };

#define _pc(exitCode) (TResult(_partially + (exitCode)))

/* Outcomes 6-15 are reserved for future use. */
const std::string outcomes[] = {"accepted",         "wrong-answer", "presentation-error", "fail",     "fail",
#ifndef PCMS2
                                "points",
#else
                                "relative-scoring",
#endif
                                "reserved",         "reserved",     "unexpected-eof",     "reserved", "reserved",
...
public:
  ...
  void setTestCase(int) { __testlib_fail("setTestCase not implemented in StringInputStreamReader"); }

  std::vector<int> getReadChars() { __testlib_fail("getReadChars not implemented in StringInputStreamReader"); }
...
private:
  ...
  bool refill() {
    ...
    if (bufferPos >= int(bufferSize)) {
      size_t readSize = fread(buffer + MAX_UNREAD_COUNT, 1, BUFFER_SIZE - MAX_UNREAD_COUNT, file);
...
public:
  ...
  std::vector<int> getReadChars() { __testlib_fail("getReadChars not implemented in BufferedFileInputStreamReader"); }
...
struct InStream {
  ...
  readUnsignedLongs(int size, unsigned long long minv, unsigned long long maxv, const std::string& variablesName = "", int indexBase = 1);
  ...
  std::vector<unsigned long long>
  readLongs(int size, unsigned long long minv, unsigned long long maxv, const std::string& variablesName = "", int indexBase = 1);
   ...
   */
  double
  readStrictReal(double minv, double maxv, int minAfterPointDigitCount, int maxAfterPointDigitCount, const std::string& variableName = "");

  std::vector<double> readStrictReals(
      int                size,
      double             minv,
      double             maxv,
   ...
   * and number is in the form "[-]digit(s)[.digit(s)]".
   */
  double readStrictDouble(
      double minv, double maxv, int minAfterPointDigitCount, int maxAfterPointDigitCount, const std::string& variableName = "");

  std::vector<double> readStrictDoubles(
      int                size,
      double             minv,
      double             maxv,
      int                minAfterPointDigitCount,
...
public:
  ...
  std::string testset() const {
    if (!_initialized)
      __testlib_fail(
          "Validator should be initialized with registerValidation(argc, argv) instead of registerValidation() to support "
          "validator.testset()");
    return _testset;
  }
  ...
  std::string testOverviewLogFileName() const { return _testOverviewLogFileName; }
  ...
  void setTestOverviewLogFileName(const char* const testOverviewLogFileName) { _testOverviewLogFileName = testOverviewLogFileName; }

  void setTestMarkupFileName(const char* const testMarkupFileName) { _testMarkupFileName = testMarkupFileName; }
  ...
  void addBoundsHit(const std::string& variableName, ValidatorBoundsHit boundsHit) {
    if (isVariableNameBoundsAnalyzable(variableName) && _boundsHitByVariableName.size() < VALIDATOR_MAX_VARIABLE_COUNT) {
      std::string preparedVariableName = prepVariableName(variableName);
      _boundsHitByVariableName[preparedVariableName] =
          boundsHit.merge(_boundsHitByVariableName[preparedVariableName], ignoreMinBound(variableName), ignoreMaxBound(variableName));
    }
  }

  void addVariable(const std::string& variableName) {
    if (isVariableNameBoundsAnalyzable(variableName) && _variables.size() < VALIDATOR_MAX_VARIABLE_COUNT) {
      std::string preparedVariableName = prepVariableName(variableName);
  ...
  void adjustConstantBounds(const std::string& variableName, T lower, T upper) {
    if (isVariableNameBoundsAnalyzable(variableName) && _constantBoundsByVariableName.size() < VALIDATOR_MAX_VARIABLE_COUNT) {
      std::string preparedVariableName = prepVariableName(variableName);
  ...
  std::string getBoundsHitLog() {
    std::string result;
    for (std::map<std::string, ValidatorBoundsHit>::iterator i = _boundsHitByVariableName.begin(); i != _boundsHitByVariableName.end();
         i++) {
      result += "\"" + i->first + "\":";
      if (i->second.minHit)
  ...
  std::string getFeaturesLog() {
    std::string result;
    for (std::set<std::string>::iterator i = _features.begin(); i != _features.end(); i++) {
      result += "feature \"" + *i + "\":";
  ...
  void writeTestOverviewLog() {
    if (!_testOverviewLogFileName.empty()) {
      ...
      fprintf(
          f, "%s%s%s%s", getBoundsHitLog().c_str(), getFeaturesLog().c_str(), getConstantBoundsLog().c_str(), getVariablesLog().c_str());
      std::fflush(f);
      if (!standard_file)
        if (std::fclose(f))
...
static std::string __testlib_appendMessage(const std::string& message, const std::string& extra) {
  ...
  if (openPos != -1 && openPos != INT_MAX && closePos != -1 && closePos != INT_MAX && openPos < closePos) {
  ...
  if (!p.matches(_tmpReadToken)) {
    if (readManyIteration == NO_INDEX) {
      if (variableName.empty())
        ...
        quit(
            _wa, ("Token parameter [name=" + variableName + "] equals to \"" + __testlib_part(_tmpReadToken)
                  + "\", doesn't correspond to pattern \"" + p.src() + "\"")
                     .c_str());
    } else {
      if (variableName.empty())
        quit(
            _wa, ("Token element [index=" + vtos(readManyIteration) + "] equals to \"" + __testlib_part(_tmpReadToken)
                  + "\" doesn't correspond to pattern \"" + p.src() + "\"")
                     .c_str());
      else
        quit(
            _wa, ("Token element " + variableName + "[" + vtos(readManyIteration) + "] equals to \"" + __testlib_part(_tmpReadToken)
                  + "\", doesn't correspond to pattern \"" + p.src() + "\"")
                     .c_str());
    }
  }
...
void InStream::readWordTo(std::string& result, const pattern& p, const std::string& variableName) {
  ...
  if (!p.matches(result)) {
    if (variableName.empty())
      ...
      quit(
          _wa, ("Token parameter [name=" + variableName + "] equals to \"" + __testlib_part(result) + "\", doesn't correspond to pattern \""
                + p.src() + "\"")
                   .c_str());
  }
  if (strict && !variableName.empty())
...
static inline double stringToDouble(InStream& in, const char* buffer) {
  ...
  for (size_t i = 0; i < length; i++) {
    if (('0' <= buffer[i] && buffer[i] <= '9') || buffer[i] == '.' || buffer[i] == 'e' || buffer[i] == 'E' || buffer[i] == '-'
        || buffer[i] == '+') {
...
static inline double stringToStrictDouble(InStream& in, const char* buffer, int minAfterPointDigitCount, int maxAfterPointDigitCount) {
  ...
  if (afterDigitsCount < minAfterPointDigitCount || afterDigitsCount > maxAfterPointDigitCount)
    in.quit(
        _pe, ("Expected strict double with number of digits after point in range [" + vtos(minAfterPointDigitCount) + ","
              + vtos(maxAfterPointDigitCount) + "], but \"" + __testlib_part(buffer) + "\" found")
                 .c_str());
  ...
  if (buffer[firstDigitPos] == '0' && firstDigitPos + 1 < int(length) && buffer[firstDigitPos + 1] >= '0'
      && buffer[firstDigitPos + 1] <= '9')
    in.quit(_pe, ("Expected strict double, but \"" + __testlib_part(buffer) + "\" found").c_str());
...
stringToStrictDouble(InStream& in, const std::string& buffer, int minAfterPointDigitCount, int maxAfterPointDigitCount) {
  for (size_t i = 0; i < buffer.length(); i++)
    if (buffer[i] == '\0')
      in.quit(_pe, ("Expected double, but \"" + __testlib_part(buffer) + "\" found (it contains \\0)").c_str());
  ...
  if (result < minv || result > maxv) {
    if (readManyIteration == NO_INDEX) {
      if (variableName.empty())
        quit(
            _wa,
            ("Integer " + vtos(result) + " violates the range [" + toHumanReadableString(minv) + ", " + toHumanReadableString(maxv) + "]")
                .c_str());
      else
        quit(
            _wa, ("Integer parameter [name=" + std::string(variableName) + "] equals to " + vtos(result) + ", violates the range ["
                  + toHumanReadableString(minv) + ", " + toHumanReadableString(maxv) + "]")
                     .c_str());
    } else {
      if (variableName.empty())
        quit(
            _wa, ("Integer element [index=" + vtos(readManyIteration) + "] equals to " + vtos(result) + ", violates the range ["
                  + toHumanReadableString(minv) + ", " + toHumanReadableString(maxv) + "]")
                     .c_str());
      else
        quit(
            _wa, ("Integer element " + std::string(variableName) + "[" + vtos(readManyIteration) + "] equals to " + vtos(result)
...
unsigned long long InStream::readUnsignedLong(unsigned long long minv, unsigned long long maxv, const std::string& variableName) {
  ...
  if (result < minv || result > maxv) {
    if (readManyIteration == NO_INDEX) {
      if (variableName.empty())
        quit(
            _wa, ("Unsigned integer " + vtos(result) + " violates the range [" + toHumanReadableString(minv) + ", "
                  + toHumanReadableString(maxv) + "]")
                     .c_str());
      else
        quit(
            _wa, ("Unsigned integer parameter [name=" + std::string(variableName) + "] equals to " + vtos(result) + ", violates the range ["
                  + toHumanReadableString(minv) + ", " + toHumanReadableString(maxv) + "]")
                     .c_str());
    } else {
      if (variableName.empty())
        quit(
            _wa, ("Unsigned integer element [index=" + vtos(readManyIteration) + "] equals to " + vtos(result) + ", violates the range ["
                  + toHumanReadableString(minv) + ", " + toHumanReadableString(maxv) + "]")
                     .c_str());
      else
        quit(
            _wa, ("Unsigned integer element " + std::string(variableName) + "[" + vtos(readManyIteration) + "] equals to " + vtos(result)
                  + ", violates the range [" + toHumanReadableString(minv) + ", " + toHumanReadableString(maxv) + "]")
                     .c_str());
    }
  }
...
std::vector<unsigned long long>
InStream::readUnsignedLongs(int size, unsigned long long minv, unsigned long long maxv, const std::string& variablesName, int indexBase) {
  if (strict && !variablesName.empty())
    validator.addVariable(variablesName);
...
std::vector<int> InStream::readInts(int size, int indexBase){__testlib_readMany(readInts, readInt(), int, true)}
...
double InStream::readReal(double minv, double maxv, const std::string& variableName) {
  ...
  if (result < minv || result > maxv) {
    if (readManyIteration == NO_INDEX) {
      if (variableName.empty())
        ...
        quit(
            _wa, ("Double parameter [name=" + std::string(variableName) + "] equals to " + vtos(result) + ", violates the range ["
                  + vtos(minv) + ", " + vtos(maxv) + "]")
                     .c_str());
    } else {
      if (variableName.empty())
        quit(
            _wa, ("Double element [index=" + vtos(readManyIteration) + "] equals to " + vtos(result) + ", violates the range [" + vtos(minv)
                  + ", " + vtos(maxv) + "]")
                     .c_str());
      else
        quit(
            _wa, ("Double element " + std::string(variableName) + "[" + vtos(readManyIteration) + "] equals to " + vtos(result)
  ...
  if (strict && !variableName.empty()) {
    validator.addBoundsHit(
        variableName,
        ValidatorBoundsHit(doubleDelta(minv, result) < ValidatorBoundsHit::EPS, doubleDelta(maxv, result) < ValidatorBoundsHit::EPS));
    validator.adjustConstantBounds(variableName, minv, maxv);
    validator.addVariable(variableName);
  }
...
double InStream::readStrictReal(
    double minv, double maxv, int minAfterPointDigitCount, int maxAfterPointDigitCount, const std::string& variableName) {
  if (!strict && seekEof())
  ...
  if (result < minv || result > maxv) {
    if (readManyIteration == NO_INDEX) {
      if (variableName.empty())
        ...
        quit(
            _wa, ("Strict double parameter [name=" + std::string(variableName) + "] equals to " + vtos(result) + ", violates the range ["
                  + vtos(minv) + ", " + vtos(maxv) + "]")
                     .c_str());
    } else {
      if (variableName.empty())
        quit(
            _wa, ("Strict double element [index=" + vtos(readManyIteration) + "] equals to " + vtos(result) + ", violates the range ["
                  + vtos(minv) + ", " + vtos(maxv) + "]")
                     .c_str());
      else
        quit(
            _wa, ("Strict double element " + std::string(variableName) + "[" + vtos(readManyIteration) + "] equals to " + vtos(result)
                  + ", violates the range [" + vtos(minv) + ", " + vtos(maxv) + "]")
                     .c_str());
    }
  }
...
std::vector<double> InStream::readStrictReals(
    int                size,
    double             minv,
    double             maxv,
    int                minAfterPointDigitCount,
  ...
  if (strict && !variablesName.empty())
    validator.addVariable(variablesName);
  __testlib_readMany(
      readStrictReals, readStrictReal(minv, maxv, minAfterPointDigitCount, maxAfterPointDigitCount, variablesName), double, true)
}

double InStream::readStrictDouble(
    double minv, double maxv, int minAfterPointDigitCount, int maxAfterPointDigitCount, const std::string& variableName) {
  return readStrictReal(minv, maxv, minAfterPointDigitCount, maxAfterPointDigitCount, variableName);
}
...
std::vector<double> InStream::readStrictDoubles(
  ...
  if (strict && !variablesName.empty())
    validator.addVariable(variablesName);
  __testlib_readMany(
      readStrictDoubles, readStrictDouble(minv, maxv, minAfterPointDigitCount, maxAfterPointDigitCount, variablesName), double, true)
}
  ...
  if (!p.matches(result)) {
    if (readManyIteration == NO_INDEX) {
      if (variableName.empty())
        ...
        quit(
            _wa, ("Line [name=" + variableName + "] equals to \"" + __testlib_part(result) + "\", doesn't correspond to pattern \""
                  + p.src() + "\"")
                     .c_str());
    } else {
      if (variableName.empty())
        quit(
            _wa, ("Line element [index=" + vtos(readManyIteration) + "] equals to \"" + __testlib_part(result)
                  + "\" doesn't correspond to pattern \"" + p.src() + "\"")
                     .c_str());
      else
        quit(
            _wa, ("Line element " + std::string(variableName) + "[" + vtos(readManyIteration) + "] equals to \"" + __testlib_part(result)
                  + "\", doesn't correspond to pattern \"" + p.src() + "\"")
                     .c_str());
    }
  }
...
void registerGen(int argc, char* argv[]) {
  std::fprintf(
      stderr, "Use registerGen(argc, argv, 0) or registerGen(argc, argv, 1)."
              " The third parameter stands for the random generator version."
              " If you are trying to compile old generator use macro -DUSE_RND_AS_BEFORE_087 or registerGen(argc, argv, 0)."
              " Version 1 has been released on Spring, 2013. Use it to write new generators.\n\n");
  registerGen(argc, argv, 0);
...
void registerInteraction(int argc, char* argv[]) {
  ...
  if (argc < 3 || argc > 6) {
    quit(
        _fail, std::string("Program must be run with the following arguments: ")
    ...
    if (!strcmp("--testCase", argv[i])) {
      if (i + 1 < argc) {
        ...
        if (testCase < 1 || testCase >= __TESTLIB_MAX_TEST_CASE)
          quit(
              _fail, testlib_format_("Argument testCase should be between 1 and %d, but ", __TESTLIB_MAX_TEST_CASE) + toString(testCase)
                         + " found");
...
public:
  ...
  std::string testset() const {
    if (!_initialized)
      __testlib_fail(
          "Checker should be initialized with registerTestlibCmd(argc, argv) instead of registerTestlibCmd() to support checker.testset()");
    return _testset;
  }
  ...
  std::string group() const {
    if (!_initialized)
      __testlib_fail(
          "Checker should be initialized with registerTestlibCmd(argc, argv) instead of registerTestlibCmd() to support checker.group()");
    return _group;
  }
...
void srand(unsigned int seed) RAND_THROW_STATEMENT {
  quitf(
      ...
      "'registerGen(argc, argv, 1);' to initialize generator seed "
      "by hash code of the command line params. The third parameter "
      "is randomGeneratorVersion (currently the latest is 1) [ignored seed=%u].",
      seed);
}
...
NORETURN void __testlib_expectedButFound(TResult result, std::string expected, std::string found, const char* prepend) {
  std::string message;
  if (strlen(prepend) != 0)
    message = testlib_format_(
        "%s: expected '%s', but found '%s'", compress(prepend).c_str(), compress(expected).c_str(), compress(found).c_str());
  else
    message = testlib_format_("expected '%s', but found '%s'", compress(expected).c_str(), compress(found).c_str());
  quit(result, message);
}
