...
struct SegmentTreeMinAddSearch {
  ...
  void init(I32 size, T initial_value = T{}) {
    ...
    Vec<T> values(size, initial_value);
    init(values);
  }
  ...
  void init(const Vec<T>& values) {
    ...
    min_val.assign(tree_size, T{});
    max_val.assign(tree_size, T{});
    lazy.assign(tree_size, T{});
...
private:
  ...
  inline void pull(I32 node) {
    min_val[node] = std::min(
        min_val[node << 1],
        min_val[node << 1 | 1]);
    max_val[node] = std::max(
        max_val[node << 1],
        max_val[node << 1 | 1]);
  }

  inline void push(I32 node) {
    const T lz = lazy[node];
    if (lz == T{}) return;
    apply(node << 1, lz);
    apply(node << 1 | 1, lz);
    lazy[node] = T{};
  ...
  T range_min_impl(I32 node, I32 tl, I32 tr, I32 ql, I32 qr) {
    ...
    if (ql <= tl && tr <= qr) return min_val[node];
  ...
  T range_max_impl(I32 node, I32 tl, I32 tr, I32 ql, I32 qr) {
    if (qr < tl || tr < ql) return -infinity<T>;
    if (ql <= tl && tr <= qr) return max_val[node];
  ...
  I32 find_rightmost_eq_impl(I32 node, I32 tl, I32 tr, I32 ql, I32 qr, T target) {
    ...
    if (target < min_val[node] || target > max_val[node]) {
      return -1;
    }
    if (tl == tr) return tl;
  ...
  I32 find_rightmost_leq_impl(I32 node, I32 tl, I32 tr, I32 ql, I32 qr, T target) {
    if (qr < tl || tr < ql || min_val[node] > target) return -1;
    if (tl == tr) return tl;
