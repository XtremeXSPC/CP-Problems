...
struct PrefixAutomaton {
  ...
  void build(const String& p) {
    ...
    const I32 n = isz(pattern);
    next_state_table.assign((n + 1), Row{});
    ...
    FOR(i, n + 1) {
      FOR(c, ALPHABET) {
        ...
        if (i < n && pattern[i] == ch) {
          next_state_table[i][c] = i + 1;
        } else if (i == 0) {
          next_state_table[i][c] = 0;
        } else {
          next_state_table[i][c] = next_state_table[pi[i - 1]][c];
        }
      }
    }
  }
